...
function createRenderer() {
  ...
  function ensureNvidiaTexture(mode) {
    ...
    transform.src = mode === "protan" ? "/nvidia_protanopia.png" : "/nvidia_deuteranopia.png";
    return false;
  }
...
async function downloadVpk() {
  if (!buildToken) {
    showStatus("Builder connection is not ready. Reload the page.", "error");
    return;
  }
  ...
  try {
    const response = await fetch("/api/build", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Build-Token": buildToken },
...
async function initializeSession() {
  try {
    ...
    buildToken = (await response.json()).token;
  } catch (error) {
    showStatus(`Builder connection failed: ${error.message}`, "error");
  }
}
...
async function initializeImages() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    previewImages = await response.json();
    if (!Array.isArray(previewImages) || previewImages.length === 0) throw new Error("No demo images found");
